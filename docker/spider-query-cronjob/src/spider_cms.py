...
from history import process_histories
from queues import query_schedd_queue
...
def main_driver(args):
    ...
    # TODO: Check if we need to collect metadata
...
def main():
    ...
    """
    # TODO: Create a class for type hinting and validation of arguments
    ...
    parser.add_argument(
        "--history_query_max_n_minutes",
        default=12 * 60,
        type=int,
        dest="history_query_max_n_minutes",
    ...
    if (not os.getenv("CMS_HTCONDOR_PRODUCER")) or (
        not os.getenv("CMS_HTCONDOR_BROKER")
    ):
    ...
    if args.feed_amq:
        ...
        if (not os.path.exists(os.path.join(_workdir, "etc/amq_username"))) or (
            ...
            logging.error(
                "Please provide [etc/amq_username] and [etc/amq_password], because feed_amq is set."
            )
            sys.exit(1)
