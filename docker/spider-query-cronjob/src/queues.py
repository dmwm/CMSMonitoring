...
from nats_client import get_nats_connection, publish_job_to_nats


def query_schedd_queue(starttime, schedd_ads, args):
    """
    ...
    count = 0
    published_count = 0
    # There is a log check in line 117 to report progress periodically, the following variable is here for that.
    ...
    for schedd_ad in schedd_ads:
        ...
        if time_remaining(starttime) < 10:
            message = (
                "No time remaining to run queue crawler on %s; exiting."
                % schedd_ad["Name"]
            )
        ...
        try:
            ...
            for job_ad in query_iter:
                ...
                if count_since_last_report >= 1000:
                    ...
                    processing_rate = (
                        count_since_last_report / cpu_usage if cpu_usage > 0 else 0
                    )
