...
def get_nats_connection(args):
    ...
    if _nats_connection is None or _nats_connection.is_closed:
        nats_servers = getattr(args, 'nats_servers', None) or os.getenv('NATS_SERVERS', 'nats://localhost:4222')
        ...
        try:
            ...
            _nats_connection = loop.run_until_complete(nats.connect(servers=nats_servers))
            _nats_jetstream = _nats_connection.jetstream()
            
            # Ensure the stream exists
            stream_name = getattr(args, 'nats_stream_name', None) or os.getenv('NATS_STREAM_NAME', 'CMS_HTCONDOR_QUEUE')
            ...
            try:
                ...
                loop.run_until_complete(_nats_jetstream.add_stream(
                    name=stream_name,
                    subjects=[subject]
                ))
