...
from affiliations import (
    get_affiliations_dict,
    get_affiliation_by_dn,
...
# Initialize affiliations dict
affiliations_dict = None
try:
    affiliations_dict = get_affiliations_dict()
except AffiliationException as e:
    # If its not possible to load affiliations
    # Log it
    logging.error("There were an error loading affiliations, %s", e)
...
def process_ad(
    ...
    # Keep JobStatus for backwards compatibility with recordTime function.
    result["JobStatus"] = result["Status"]
    ...
    if affiliations_dict:
        ...
        if "CRAB_UserHN" in result:
            _aff = affiliations_dict.get(result["CRAB_UserHN"])
        elif "x509userproxysubject" in result:
            _aff = get_affiliation_by_dn(affiliations_dict, result["x509userproxysubject"])
...
def recordTime(ad):
    ...
    if "JobStatus" in ad:
        ...
        job_status = ad["Status"]
    if job_status in [3, 4, 6]:
...
def bulk_convert_ad_data(ad, result):
    ...
    for key in _keys:
        ...
        if isinstance(value, classad.Value):
            ...
            # For uncategorized fields, skip "Unknown" string values to avoid
            # OpenSearch mapping conflicts (e.g. float fields receiving a string).
            if isinstance(value, str) and value.lower() == "unknown":
                value = None
                logging.debug("Skipping unknown value for key %s: %s", key, value)
...
def evaluate_fields(result, ad):
    ...
    if "RequestCpus" in ad:
        try:
            result["RequestCpus_Eval"] = ad.eval("RequestCpus")
        except Exception as eval_exc:
            logging.error(
                "Could not evaluate RequestCpus expression, error: %s" % (str(eval_exc))
            )
...
def unique_doc_id(doc):
    ...
    """
    return "%s#%d" % (doc["GlobalJobId"], recordTime(doc))
