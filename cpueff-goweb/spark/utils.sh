#!/bin/bash
# Utils for cpueff-goweb scripts
...
function util_input_args_parser() {
    unset -v KEYTAB_SECRET PORT1 PORT2 K8SHOST WDIR OUTPUT_DIR LAST_N_DAYS IS_K8S IS_TEST help
    # Dictionary to keep variables
    declare -A arr

    PARSED_ARGS=$(getopt --unquoted --options v,h --name "$(basename -- "$0")" --longoptions keytab:,p1:,p2:,host:,wdir:,output:,lastndays:,k8s,test,help -- "$@")
    VALID_ARGS=$?
