...
def get_rdd_schema():
    ...
    return StructType(
        [
            ...
            StructField(
                "acquisition_era", ArrayType(StringType(), False), nullable=True
            ),
    ...
    if "steps" in row and row["steps"] is not None:
        for step in filter(None, row["steps"]):
            if "name" in step and step["name"].lower().startswith("cmsrun"):
                ...
                if (
                    step_res["nthreads"]
                    and step_res["cpu_time"]
                    and step_res["job_time"]
                ):
                    try:
                        step_res["threads_total_job_time"] = (
                            step_res["job_time"] * step_res["nthreads"]
                        )
                    except Exception:
                ...
                if "performance" in step and step["performance"] is not None:
                    ...
                    if "storage" in performance:
                        if "writeTotalMB" in performance["storage"]:
                            step_res["write_total_mb"] = performance["storage"][
                                "writeTotalMB"
                            ]
                        if "readTotalMB" in performance["storage"]:
                            step_res["read_total_mb"] = performance["storage"][
                                "readTotalMB"
                            ]
                        if (
                            "writeTotalSecs" in performance["storage"]
                            and performance["storage"]["writeTotalSecs"]
                        ):
                            step_res["write_total_secs"] = float(
                                performance["storage"]["writeTotalSecs"]
                            )
                        if (
                            "readTotalSecs" in performance["storage"]
                            and performance["storage"]["readTotalSecs"]
                        ):
                            step_res["read_total_secs"] = float(
                                performance["storage"]["readTotalSecs"]
                            )
                    if "memory" in performance:
                        ...
                        if "PeakValueVsize" in performance["memory"]:
                            step_res["peak_v_size"] = performance["memory"][
                                "PeakValueVsize"
                            ]
...
@click.option(
    ...
    type=str,
    required=True,
    help="I.e. /tmp/${KERBEROS_USER}/prod/cpueff ",
)
@click.option(
    "--last_n_days", type=int, default=30, help="Last n days data will be used"
)
def main(start_date, end_date, hdfs_out_dir, last_n_days):
    ...
    click.echo(
        "Stepchain cpu efficiency results to HDFS temp directory which will be imported to MongoDB from there"
    )
    click.echo(
        f"Input Arguments: start_date:{start_date}, end_date:{end_date}, "
    ...
    hdfs_out_collection_dirs = {
        c: hdfs_out_dir + "/" + c for c in mongo_collection_names
    }
    ...
    df_raw = (
        ...
        .filter(
            (
                (col("data.meta_data.jobstate") == "success")
                & (col("data.wmats") >= start_date.timestamp())
                & (col("data.wmats") < end_date.timestamp())
                & col("data.task").isNotNull()
                & col("data.meta_data.jobtype").isin(_PROD_CMS_JOB_TYPES_FILTER)
            )
        )
    )
    ...
    df = (
        spark.createDataFrame(df_rdd, schema=get_rdd_schema())
        .dropDuplicates()
        .where(col("nstreams").isNotNull())
    )

    df_task = (
        df.groupby(["Task"])
        .agg(
            (100 * _sum("cpu_time") / _sum("threads_total_job_time")).alias(
                "CpuEfficiency"
            ),
            ...
            (_sum("threads_total_job_time") / _HOUR_DENOM).alias(
                "TotalThreadJobTimeHr"
            ),
            (_sum("write_total_secs") / _HOUR_DENOM).alias("WriteTotalHr"),
            (_sum("read_total_secs") / _HOUR_DENOM).alias("ReadTotalHr"),
            (100 * _sum("read_total_secs") / _sum("job_time")).alias(
                "ReadTimePercentage"
            ),
            _sum("write_total_mb").alias("WriteTotalMB"),
            _sum("read_total_mb").alias("ReadTotalMB"),
            _mean(when(col("peak_rss").isNotNull(), col("peak_rss"))).alias(
                "MeanPeakRss"
            ),
            _mean(when(col("peak_v_size").isNotNull(), col("peak_v_size"))).alias(
                "MeanPeakVSize"
            ),
            array_distinct(flatten(collect_set(col("acquisition_era")))).alias(
                "AcquisitionEra"
            ),
    ...
    df_task_cmsrun_jobtype = (
        df.groupby(["Task", "StepName", "JobType"])
        .agg(
            (100 * _sum("cpu_time") / _sum("threads_total_job_time")).alias(
                "CpuEfficiency"
            ),
    ...
    df_task_cmsrun_jobtype_site = (
        df.groupby(["Task", "StepName", "JobType", "Site"])
        .agg(
            (100 * _sum("cpu_time") / _sum("threads_total_job_time")).alias(
                "CpuEfficiency"
            ),
    ...
    df_task.write.save(
        path=hdfs_out_collection_dirs["sc_task"], format=write_format, mode=write_mode
    )
    df_task_cmsrun_jobtype.write.save(
        path=hdfs_out_collection_dirs["sc_task_cmsrun_jobtype"],
        format=write_format,
        mode=write_mode,
    )
    df_task_cmsrun_jobtype_site.write.save(
        path=hdfs_out_collection_dirs["sc_task_cmsrun_jobtype_site"],
        format=write_format,
        mode=write_mode,
    )
    ...
    for mongo_col in mongo_collection_names:
        ...
        os.system(
            f"hadoop fs -getmerge {hdfs_out_collection_dirs[mongo_col]}/part-*.json {mongo_col}.json"
        )
        # Send local json file to MongoDB
        mongoimport_cmd_prefix = (
            f"/data/mongoimport --drop --type=json --port {mongo_port} "
            f"--host {mongo_host} --username {mongo_u} --password {mongo_p} "
            f"--authenticationDatabase {mongo_auth_db} --db {mongo_db} "
        )
        os.system(
            mongoimport_cmd_prefix + f"--collection {mongo_col} --file {mongo_col}.json"
        )
