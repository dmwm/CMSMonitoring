...
parser.add_argument("end_date", help="Date in yyyyMMdd format.", nargs='?', type=str, default=datetime.strftime(datetime.now() - relativedelta(days=1), '%Y%m%d'))
...
start_date_d = datetime.strptime(args.end_date, '%Y%m%d')  - relativedelta(months=12, days=-1)
...
phedexInput = os.path.join(base, "phedex/phedex_{}_{}.csv.gz".format(start_date, end_date))
eventsInput = os.path.join(base, "dbs_events.csv.gz")

# HDFS location of dbs events input dataframe 
dbsInput = "hdfs:///cms/dbs_condor/dataset/201*/*/*/part-*"
...
dbs_df = spark.read.csv(dbsInput, header=True).select(u'dataset', 'sum_evts') .withColumn('filename',input_file_name())
