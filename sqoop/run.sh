...
if [ ! -e "${CMSSQOOP_CONFIGS}" ]; then
    ...
    if [ $CMSSQOOP_ENV = "prod" ]; then
        # If no CMSSQOOP_CONFIGS provided and CMSSQOOP_ENV is provided as prod, set production paths configs.json
        export CMSSQOOP_CONFIGS=$script_dir/configs.json
    else
        export CMSSQOOP_CONFIGS=$script_dir/configs-test.json
    fi
fi
