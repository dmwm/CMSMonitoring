...
def get_df_dbs_f_d(spark):
    ...
    df_dbs_f_d = (
        dbs_files.join(dbs_datasets, ["DATASET_ID"], how="left")
...
def get_df_ds_general_info(spark):
    ...
    return (
        dbs_datasets.join(dbs_files, ["DATASET_ID"], how="left")
        .groupby("DATASET_ID")
        .agg(
            _sum("FILE_SIZE").alias("RealSize"),
            countDistinct(col("LOGICAL_FILE_NAME")).alias("TotalFileCnt"),
        )
        .withColumnRenamed("DATASET_ID", "Id")
        .select(["Id", "RealSize", "TotalFileCnt"])
    )
...
def get_df_dataset_file_rse_ts_size(df_file_rse_ts_size, df_dbs_f_d):
    ...
    """
    df_dataset_file_rse_ts_size = (
        df_file_rse_ts_size.join(df_dbs_f_d, ["f_name"], how="left")
...
def get_df_enr_with_rse_info(df_dataset_file_rse_ts_size, df_rses):
    ...
    return df_dataset_file_rse_ts_size.join(df_rses, ["rse_id"], how="left").select(
        [
            "dataset_id",
            "dataset",
            "f_name",
...
def get_df_sub_rse_details(df_enr_with_rse_info):
    ...
    """
    return (
        df_enr_with_rse_info.groupby(["rse_id", "dataset"])
        .agg(
            _sum(col("f_size")).alias("SizeInRseBytes"),
...
def get_df_main_datasets(df_sub_rse_details, df_ds_general_info):
    ...
    """
    df = (
        df_sub_rse_details.groupby(["RseType", "Dataset"])
        .agg(
            _max(col("SizeInRseBytes")).cast(LongType()).alias("Max"),
    ...
    return df.join(df_ds_general_info, ["Id"], how="left").select(
        [
            "Id",
            "RseType",
            "Dataset",
...
def main(hdfs_out_dir):
    ...
    df_dataset_file_rse_ts_size = get_df_dataset_file_rse_ts_size(
        df_file_rse_ts_size, df_dbs_f_d
    )
    df_enr_with_rse_info = get_df_enr_with_rse_info(
        df_dataset_file_rse_ts_size, df_rses
    )
